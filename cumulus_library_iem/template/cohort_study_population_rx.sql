...
--   * COALESCE(medicationrequest_ref, id) avoids collapsing NULL-ref
--     rows into a single ROW_NUMBER partition.
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                ...
                DATE_DIFF(
                    ...
                    COALESCE(
                        study_population.enc_period_end_day,
                        study_population.enc_period_start_day
                    )
                ) ASC,
                ABS(
...
rx_by_authoredon AS (
    ...
    JOIN core__medicationrequest AS rx
        ON COALESCE(rx.medicationrequest_ref, rx.id) = match.rx_key
